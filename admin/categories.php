...
if ($arg == "1") {
  ...
  $sql = "SELECT MAX(cat_seq) AS follow FROM site_categories " .
         "WHERE cat_seq < '$seq'";
  $result = mysql_query($sql, $connect);
  if ($row = mysql_fetch_object($result)) {
    $follow = $row->follow;
...
<?php
$sql = "SELECT * FROM site_categories " .
       "WHERE cat_classid='$classid' " .
       "ORDER BY cat_seq";
$result = mysql_query($sql, $connect);
while ($row = mysql_fetch_object($result)) {
  $selected = $row->cat_id == $id ? "selected" : "";
  echo "<option $selected value='$row->cat_id'>$row->cat_category</option>\n";
}
?>
      </select>
...
while ($row = mysql_fetch_object($result)) {
  ...
  $selected = $row->cat_seq == $follow ? "selected" : "";
  echo "<option $selected value='$row->cat_id'>$row->cat_category</option>\n";
}
?>
      </select>
